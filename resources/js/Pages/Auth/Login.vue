...
const submitPasswordForm = () => {
    passwordForm.post(route('password.email'), {
        onSuccess: () => {
        ...
        <section class="bg-gray-50 dark:bg-gray-900">
            <div class="flex flex-col items-center justify-center py-8 mx-auto md:h-screen lg:py-0 w-[420px]">
                ...
                <div
                    ...
                    <div class="px-6 py-8 space-y-4 md:space-y-6 sm:p-8">
                            ...
                            <div class="space-y-4">
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        ...
                                        />
                                        <label for="email"
                                               class="cursor-pointer absolute inset-y-0 right-0 pr-3 flex items-center text-sm leading-5">
                                    ...
                                    <p class="mt-2 font-medium text-sm text-red-600 dark:text-red-500"
                                       v-if="form.errors.email">{{ form.errors.email }}</p>
                                </div>
                                <div class="space-y-2">
                                    ...
                                    <p class="mt-2 font-medium text-sm text-red-600 dark:text-red-500"
                                       v-if="form.errors.password">{{ form.errors.password }}</p>
                                </div>
                            </div>
                            <div class="flex flex-col items-end h-6 mt-2 mb-4">
            ...
            <Modal :show="confirmingPasswordForgotten" :closeable="true" @close="closeModal">
                <div class="w-full flex flex-col gap-8 relative text-center" v-if="!showModal">
                    ...
                    <form @submit.prevent="submitPasswordForm" class="flex flex-col items-center">
                        <div class="flex flex-col items-start w-full">
                            ...
                            <label for="email"
                                   class="block mb-2 text-base font-semibold dark:text-white"> {{
                                    $t("labels.email")
                                }}</label>
                ...
                <div class="w-full flex flex-col gap-8 text-center" v-else>
                    ...
                    <p class="text-lg font-normal">
                        <span
                            v-html="$t('loginPage.passwordResetPage.emailSentMessage', { email: '<span class=\'font-bold\'>' + form.email + '</span>' })"></span>
                    </p>
                    <div class="flex flex-col items-center text-sm gap-2">
                        <div class="flex items-baseline">
                            <p class="mr-1">{{ $t('loginPage.passwordResetPage.emailNotReceived') }}</p>
                            <button class="m-link" @click="resendVerification">
                                {{ $t('loginPage.passwordResetPage.resendEmail') }}
                            </button>
                        </div>
                        <div class="flex items-baseline">
                            <p class="mr-1">{{ $t('loginPage.passwordResetPage.emailNotReceived') }}</p>
                            <button class="m-link" @click="changeEmail">
                                {{ $t('loginPage.passwordResetPage.changeEmailAddress') }}
                            </button>
                        </div>
                    </div>
                </div>
